...
let AUTH_REQUEST_VALIDATION_SCHEMA = {
    //required params
    scope: Joi.string().default("").description(AUTH_PARAM_SCOPE_DESCR),
    response_type: Joi.string().default("").description(AUTH_PARAM_RESPONSE_TYPE_DESCR),
    client_id: Joi.string().required().description(AUTH_PARAM_CLIENT_ID_DESCR),
    redirect_uri: Joi.string().uri({
        scheme: ["https"]
    }).required().description(AUTH_PARAM_REDIRECT_URI_DESCR),
...
function __verify_client_registration_entries(authorize_request) {
    ...
    return Q.Promise((resolve, reject) => {
        ...
        try {
            ...
            oidc_options.client_registrar_module.get_client_registration(authorize_request.client_id)
                .then((client_registry) => {
                    if (!client_registry ||
                        (client_registry.redirect_uri_hostname.trim() !== redirect_url.hostname) ||
                        (client_registry.redirect_uri_port.trim() !== redirect_url.port)) {
                        error_object.error_description = "Incorrect client_id/redirect_uri values";
                        return reject(error_object);
                    } else {
                        return resolve();
...
function __validate_authorization_request(authorize_request) {
    return Q.Promise((resolve, reject) => {
        ...
        let openid_scope_found = authorize_request.scope.split(" ").find((element, idx, arr) => {
 ...
 * @return {[type]}                      [description]
 */
function __process_authorization_request(request, reply, authorization_request) {
    /**
     ...
     * @return {[type]}                      [description]
     */
    __verify_client_registration_entries(authorization_request)
        .then(() => {
            /**
             ...
             */
            let processing_tasks = [__validate_authorization_request, oidc_options.authorization_request_store_module.post_authorization_request];

            processing_tasks.reduce(Q.when, Q(authorization_request))
                .then((authorization_request_id) => {
                    /**
                     * Everything is good to proceed for End-User authentication & concent:
                     */
                    let temp_redirect_uri = oidc_options.user_authentication_url + "?authorization_request_id=" + encodeURIComponent(authorization_request_id);
                    return reply.redirect(temp_redirect_uri);
            ...
            return reply(error_html).code(401).type("text/html");
        });
}
 ...
 * @private
 */
function __post_authorize(request, reply) {
    ...
    __process_authorization_request(request, reply, authorization_request);
}
 ...
 * @private
 */
function __get_authorize(request, reply) {
    let authorization_request = {};
...
module.exports.register = function(server, options, next) {
    ...
    server.route({
        method: "POST",
        path: oidc_options.oidc_url_root + "/authorize",
        config: {
