...
let AUTH_HEADER_PARAM_AUTHORIZATION = "REQUIRED except for grant_type='authorization_code'. Http header Authorization, must be of type 'Basic', containing the requester's credentials.";
...
let TOKEN_REQUEST_VALIDATION_SCHEMA = {
    ...
    username: Joi.string().description("The resource owner's user_id - rquired for the password grant_type"),
...
function __verify_and_get_token_request_params(request) {
        ...
        if ((auth_header_str_array) && (auth_header_str_array[1].trim().toUpperCase() === "BASIC")) {
            try {
                ...
                if (temp_str_arr) {
                    ...
                    temp_authorization_credentials.user_id = temp_str_arr[1].trim();
                    temp_authorization_credentials.user_password = temp_str_arr[2].trim();

                    authorization_credentials = temp_authorization_credentials;
                }
            } catch (err) {}
        }
    }
    ...
    /**
     * http authorization Basic is required except for grant_type="authorization_code"
     */
    if ((request_params.grant_type !== "authorization_code") && !authorization_credentials) {
        throw new Error("Basic authorization required for all grant_type other than authorization_code");
    }

    if (authorization_credentials) {
        request_params.authorization_credentials = authorization_credentials;
    }
...
function __send_token_for_authorization_code(request_params, request, reply) {
    authorization_endpoint.authorization_request_registrar_module.get_authorization_request(request_params.code)
...
/**
 * The credentials will be checked with the clien_registrar. If a client
 * is found, it's client_id is used for the audience.
 *
 * If no client is found to match the authorization credentials, then
 * the user account registrar is queried, and if a matching user account
 * is found, then the audience attribute of the user account is used for
 * audience, if present (defualt is THIS issuer as an audience).
 *
 * If neither client nor user accounts match the credentials, then, the
 * request is denied.
 *
 * @param  {[type]} request_params [description]
 * @return {[type]}                [description]
 */
function __process_authorization_header(request_params) {

    request_params.audience = null;
     ...
     * @return {[type]}                [description]
     */
    function __query_client_registrar(request_params) {
        return Q.Promise((resolve, reject) => {
            client_endpoint.client_registrar_module.get_client_registration(request_params.authorization_credentials.user_id)
                .then((client_account) => {
                    request_params.audience = request_params.authorization_credentials.user_id;
     ...
     * @return {[type]}                [description]
     */
    function __query_user_account_registrar(request_params) {
        if (request_params.audience) {
            return Q(request_params);
        } else if ((request_params.username !== request_params.authorization_credentials.user_id) ||
            (request_params.password !== request_params.authorization_credentials.user_password)) {
            return Q.reject(new Error("Unauthorized request"));
        }

        return Q.Promise((resolve, reject) => {
            user_info_endpoint.user_account_registrar_module.get_user_account(request_params.authorization_credentials.user_id)
                .then((user_account) => {

                    if (user_account.oidc_audience) {
                        request_params.audience = user_account.oidc_audience;
                    } else {
                        request_params.audience = [oidc_options.configuration.issuer];
                    }
                    return resolve(request_params);
                }, reject);
        });
    ...
    return [__query_client_registrar, __query_user_account_registrar].reduce(Q.when, Q(request_params));
}
...
function __send_token_for_password(request_params, request, reply) {
    ...
    __process_authorization_header(request_params)
        .then(() => {
            //username is used by the OpenID Connect spec as an id of the user, but
            //the user_id attribute name is used in this implementaion.
            user_info_endpoint.user_account_registrar_module.get_user_account(request_params.username)
                .then((user_account) => {
                    if (user_account.user_password === request_params.password) {

                        //send token for password grant_type
                        let oidc_claims = {
                            audience: [].concat(request_params.audience),
                            subject: user_account._id.toString()
                        };
                    ...
                    /**
                     * if we get here, then the user credentials did not match the ones in the user_account_registrar
                     */
                    return reply({
                        "error": "access_denied",
                        "error_description": "Invalid, denied, or expired authorization code"
    ...
    server.route({
        ...
        config: {
            ...
            validate: {
                ...
                headers: Joi.object({
                    "authorization": Joi.string().description(AUTH_HEADER_PARAM_AUTHORIZATION)
                }).options({
                    allowUnknown: true
                }),
                options: {
