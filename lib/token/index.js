...
function __process_authorization_header_for_client(request_params) {
    return Q.Promise((resolve, reject) => {
        let encrypted_request_password = client_endpoint.client_registrar_module.encrypt_password(request_params.authorization_credentials.password);
        client_endpoint.client_registrar_module.get_client_registration(request_params.authorization_credentials.username)
            .then((client_account) => {
                if (client_account.password === encrypted_request_password) {
...
function __process_authorization_header_for_user(request_params) {
    return Q.Promise((resolve, reject) => {
        let encrypted_request_password = user_info_endpoint.user_account_registrar_module.encrypt_password(request_params.authorization_credentials.password);
        user_info_endpoint.user_account_registrar_module.get_user_account(request_params.authorization_credentials.username)
            .then((user_account) => {
                if (user_account.password === encrypted_request_password) {
                    return resolve(request_params);
                } else {
                    return reject(new Error("Unauthorized request"));
                }
    ...
    authorization_header_processor.then(
            () => {
                user_info_endpoint.user_account_registrar_module.get_user_account(request_params.username)
                    .then((user_account) => {
                        let encrypt_password = user_info_endpoint.user_account_registrar_module.encrypt_password(request_params.password);
                        if (user_account.password === encrypt_password) {

                            //send token for password grant_type
                            let oidc_claims = {
                                audience: [oidc_options.configuration.issuer].concat(user_account.oidc_audience),
