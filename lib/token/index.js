...
let TOKEN_REQUEST_VALIDATION_SCHEMA = Joi.object({
    grant_type: Joi
        .string()
        .required()
        .valid("authorization_code", "password", "refresh_token")
 ...
 * @return {[type]}                [description]
 */
async function __send_token_for_authorization_code(request_params, request, h) {
 ...
 * @return {[type]}                [description]
 */
function __send_token_for_refresh_token(request_params, request, h) {
    return h
        .response({status: "Not implemented.", status_code: 501})
        .code(501)
        .type("application/json");
}
 ...
 * @return {[type]}                [description]
 */
async function __send_token_for_password(request_params, request, h) {

    let authorization_header_processor = null;
    if ((request_params.username === request_params.authorization_credentials.username) && (request_params.password === request_params.authorization_credentials.password)) {
    ...
    try {
        await authorization_header_processor;

        try {
            const user_account_id = await user_info_endpoint
                .user_account_registrar_module
                .get_user_account_id_for_credentials(request_params.username, request_params.password);
            ...
            return __send_oidc_token(request, h, oidc_claims, token_endpoint.password_grant_type.token_duration_seconds);
        } catch (err) {
            return h
                .response({"error": "access_denied", "error_description": "Invalid, denied, or expired authorization code"})
                .type("application/json")
        ...
        return h
            .response({"error": "access_denied", "error_description": "Invalid, denied, or incorrect authorization credentials"})
            .type("application/json")
            .code(401);
    }
}
 ...
 * @return {[type]}                        [description]
 */
async function __send_oidc_token(request, h, oidc_claims, token_duration_seconds) {
    ...
    let oidc_token = jwt.sign(new_token, private_key, {algorithm: "RS256"});
    ...
    try {
        const access_token = await token_endpoint
            .token_registrar_module
            .post_token(oidc_token_record);

        return h
            .response({
                "access_token": access_token, "token_type": oidc_token_record.token_type,
                //TODO: refresh_token
 ...
 * @return {[type]}         [description]
 */
function __process_token_request(request, h) {
    let request_params;
    ...
    try {
        ...
        if (!request_params.authorization_credentials) {
             ...
             */
            return h
                .response({"error": "access_denied", "error_description": "Basic authorization required for all grant_types"})
                .type("application/json")
                .header("WWW-Authenticate", "Basic realm='" + oidc_options.oidc_url_path + "'");
        }

        if (request_params.grant_type === "authorization_code") {
            return __send_token_for_authorization_code(request_params, request, h);
        } else if (request_params.grant_type === "refresh_token") {
            return __send_token_for_refresh_token(request_params, request, h);
        } else if (request_params.grant_type === "password") {
            return __send_token_for_password(request_params, request, h);
        }

    } catch (err) {
        return h
            .response({"error": "invlalid_request", "error_description": err.message})
            .type("application/json");
    }
}
 ...
 * @param options
 */
const hoc_token_plugin = async (server, options) => {
                ...
                headers: Joi
                    .object({
                        "authorization": Joi
                            .string()
                            .required()
                            .description(AUTH_HEADER_PARAM_AUTHORIZATION)
                    })
    ...
    try {
        let private_key_text = await promisify(fs.readFile)(oidc_options.configuration.jwk.priv_key_file_name, {encoding: "utf8"});
