...
function __process_authorization_header_for_client(request_params) {
    return Q.Promise((resolve, reject) => {
        client_endpoint.client_registrar_module.get_client_registration(request_params.authorization_credentials.username)
            .then((client_account) => {
                if (client_account.password === request_params.authorization_credentials.password) {
...
function __process_authorization_header_for_user(request_params) {
    return Q.Promise((resolve, reject) => {
        user_info_endpoint.user_account_registrar_module.get_user_account(request_params.authorization_credentials.username)
            .then((user_account) => {
                if (user_account.password === request_params.authorization_credentials.password) {
                    return resolve(request_params);
                } else {
                    return reject(new Error("Unauthorized request"));
                }
