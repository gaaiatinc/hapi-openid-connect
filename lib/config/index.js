...
//        || ((!oidcOptions.configuration.jwk) || (!oidcOptions.configuration.jwk.cert_type_rsa))
        || ((!oidcOptions.configuration.jwk) || (!oidcOptions.configuration.jwk.pub_cert_file_name))
        || ((!oidcOptions.configuration.jwk) || (!oidcOptions.configuration.jwk.priv_cert_file_name))
    ...
    var oidcConfiguration = {
        ...
        "authorization_endpoint": oidcOptions.configuration.issuer + appConstants.AUTHORIZATION_ENDPOINT,


        //URL of the OP's OAuth 2.0 Token Endpoint [OpenID.Core]. This is REQUIRED unless only the Implicit Flow is used.
        "token_endpoint": oidcOptions.configuration.issuer + appConstants.TOKEN_ENDPOINT,
        ...
        "userinfo_endpoint": oidcOptions.configuration.issuer + appConstants.USERINFO_ENDPOINT,
        ...
        "jwks_uri": oidcOptions.configuration.issuer + appConstants.JWKS_ENDPOINT,
        ...
        "scopes_supported": oidcOptions.configuration.scopes_supported,
     ...
     * @private
     */
    function __sendJWKSPayload(request, reply) {

        var retVal = pem2jwk(pubCert, {key_ops: ["verify", "encrypt"], kid: oidcOptions.configuration.issuer + " OIDC public key"}, "public");
        //console.log(retVal);
    ...
    server.route({
        method: "GET",
        path: oidcOptions.oidc_url_root + appConstants.JWKS_ENDPOINT,
        config: {
            handler: __sendJWKSPayload,
    ...
    server.route({
        method: "GET",
        path: oidcOptions.oidc_url_root + appConstants.JWKS_X5C_ENDPOINT,
        config: {
            handler: __sendCertificatesInPayload,
            description: appConstants.JWKS_X5C_ENDPOINT,
            notes: "OIDC x5u GET implementation",
            tags: ["api"],
            auth: null,
            plugins: {
                "hapi-auth-cookie": {
