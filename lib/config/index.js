...
module.exports.register = function (server, options, next) {
     ...
     * @private
     */
    function __sendPublicKeyInPayload(request, reply) {
        if (oidcOptions.configuration) {
    ...
    function __sendJWKSPayload(request, reply) {
        ...
        //let jwk = pem2jwk(privateKey, {key_ops: ["verify", "encrypt"], kid: oidcOptions.configuration.issuer + " OIDC public key"}, "public");
        ////console.log(jwk);
        //
        //return reply({
        //        keys: [jwk]
    ...
    server.route({
        ...
        config: {
            handler: __sendPublicKeyInPayload,
    ...
    let readKeyTasks = [__readPrivateKeyFileTask, __readPublicKeyFileTask];

    /**
     *
     * @return {[type]} [description]
     */
    function __readPrivateKeyFileTask() {
        return Q.Promise((resolve, reject) => {
            Q.nfcall(
                    fs.readFile, oidcOptions.configuration.jwk.priv_key_file_name, {
                        encoding: "utf8"
                    })
                .then((privKeyText) => {
     ...
     * @return {[type]} [description]
     */
    function __readPublicKeyFileTask() {
        return Q.Promise((resolve, reject) => {
            Q.nfcall(
                    fs.readFile, oidcOptions.configuration.jwk.pub_key_file_name, {
                        encoding: "utf8"
                    })
                .then((pubKeyText) => {
