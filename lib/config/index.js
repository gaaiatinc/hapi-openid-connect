 ...
 * @param options
 */
const hoc_config_plugin = async (server, options) => {
     ...
     * @private
     */
    function __get_oidc_configuration(request, h) {
        if (oidc_options.configuration) {
            return h
                .response(oidc_configuration)
     ...
     * @private
     */
    function __send_public_key_in_payload(request, h) {
        if (oidc_options.configuration) {
            return h
                .response(publicKey)
                .type("text/plain");
     ...
     * @private
     */
    function __send_JWKS_payload(request, h) {
    ...
    function __read_private_key_file_task() {
        return new Promise(async (resolve, reject) => {
            try {
                const privKeyText = await promisify(fs.readFile)(oidc_options.configuration.jwk.priv_key_file_name, {encoding: "utf8"});
    ...
    function __read_public_key_file_task() {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const pubKeyText = await promisify(fs.readFile)(oidc_options.configuration.jwk.pub_key_file_name, {encoding: "utf8"});
    ...
    return promiseSequencer(readKeyTasks);
};
